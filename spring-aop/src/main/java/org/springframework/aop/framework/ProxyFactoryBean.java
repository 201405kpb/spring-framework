...
public class ProxyFactoryBean extends ProxyCreatorSupport
	...
	//要提供的通知者、拦截器或其他通知名称的字符串数组. 在先到先得的服务基础上,
	// Ordering (顺序) 是重要的. 也就是说, 列表中的第一个拦截器将首先拦截调用.
	@Nullable
	...
	public Object getObject() throws BeansException {
		//在返回代理对象之前，获取到配置文件配置的所有的advisor，并且放到一个集合中，供后续目标对象的某个方法被调用时，
		//去遍历advisor链，然后根据每个advisor的Pointcut去决定执行对应的Advice
		initializeAdvisorChain();
		//判断是不是单例（一般情况下，bean中没有显示设置这个属性的话，该属性默认为true）
		if (isSingleton()) {
	 ...
	 * 创建advisor(拦截器)链。Advisors(拦截器)是通过工厂API以编程方式添加的
	 */
